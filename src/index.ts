import { v4 as uuidV4} from 'uuid'

type Task = {
  id:string,
  title:string,
  completed:boolean,
  createdAt:Date
}


const list = document.querySelector<HTMLUListElement>("#list")
const form = document.getElementById("new-task-form") as HTMLFormElement | null
const input = document.querySelector<HTMLInputElement>("#new-task-title")
const clearCheckedButton = document.getElementById("clear-checked-button");


const tasks: Task[] = loadTasks()
tasks.forEach(addListItems)



form?.addEventListener ("submit", e=>{
  e.preventDefault()

  if(input?.value == ""  || input?.value == null) return


const newTask:Task = {
  id:uuidV4(),
  title:input.value,
  completed:false,
  createdAt:new Date()
}


tasks.push(newTask)
saveTasks()


addListItems(newTask)
input.value = ""
})


function addListItems(task:Task){

const item = document.createElement("li")
const label = document.createElement("label")
const checkbox = document.createElement("input")

checkbox.addEventListener("change", ()=>{
  task.completed = checkbox.checked
  saveTasks()
})

checkbox.type = "checkbox"

checkbox.checked = task.completed

label.append(checkbox, task.title)
item.append(label)
list?.append(item)


}

function saveTasks(){
  localStorage.setItem("TASKS", JSON.stringify(tasks))
}


function loadTasks(): Task[]{
  const taskJSON = localStorage.getItem("TASKS")
  if(taskJSON == null) return []
  return JSON.parse(taskJSON)
}

clearCheckedButton?.addEventListener("click", () => {
  clearCheckedTasks();
});


function clearCheckedTasks() {
  // Filter out completed tasks and keep only the remaining tasks
  const remainingTasks = tasks.filter((task) => !task.completed);

  // Save the updated tasks list
  tasks.length = 0; // Clear the existing array
  tasks.push(...remainingTasks); // Add back the remaining tasks

  saveTasks();

  location.reload(); // Refresh the page to reflect the changes
}

